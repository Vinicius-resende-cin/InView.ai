...
Unlike agent_client.py (which talks to opencode, a third-party tool that
...
agent_client.run_agent does, so graph.py can dispatch to either backend
without the rest of the pipeline caring which one ran.
"""
...
def _run_claude_once(
    ...
    # line, regardless of subprocess's own (much higher) limit. The prompt
    # (the annotated diff) easily blows past that inline, so it goes over
    # stdin instead; the system prompt goes through a temp file via
    # --system-prompt-file, a documented but unlisted sibling of
    # --system-prompt (see --bare's help text). Only the schema stays
    ...
    # Run from this project's own directory (not source_root) so Claude Code
    # auto-discovers .claude/skills/ here; --add-dir grants read access into
    # source_root as well, without changing where skills/settings resolve
    # from - unlike opencode, no separate global-config sync is needed.
    cmd = [
        executable,
        "-p",
...
def run_agent(
    ...
    """Run one review via the claude CLI, retrying (each a fresh process) up
    to `agent_config.max_retries` extra times on invalid structured output.

    `agent_name` is accepted for interface parity with agent_client.run_agent
    ...
    for _ in range(attempts):
        result = _run_claude_once(
            llm_config, agent_config, system_text, human_text, result_model, source_root,
        )
